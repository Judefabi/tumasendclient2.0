...
const ChatList = ({chat, chatId}) => {
  ...
  const openChat = (chatId, courier, messageId, senderId, status) => {
    ...
    markRead(chatId, courier);
  };

  const markRead = (chatId, courier) => {
    // firestore()
    //   .collection('chats')
    //   .doc(chatId)
    //   .collection('messages')
  ...
  return (
    <TouchableOpacity
      ...
      {chat.courier.photoUrl !== null ? (
        ...
        <Image style={styles.avatar} source={{uri: chat.courier.photoUrl}} />
      )}
